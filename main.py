...
_embedder    = None
_cluster_data = None
THRESHOLD     = None   # set by load_models() from clusters.pkl — do not hardcode
...
def load_models():
    ...
    # Load the adaptive anomaly score threshold calibrated by build.py
    global THRESHOLD
    THRESHOLD = _cluster_data["anomaly_score_threshold"]
...
def _phase1(img_bgr):
    ...
    score       = float(np.mean(np.sort(flat)[-300:]))
...
# Explanation text
# ─────────────────────────────────────────────────────────────
def _build_explanation(defect_label, geo_dict, confidence):
    ...
    conf_str  = "" if confidence == "high" else " [low confidence — treat as approximate]"

    return (f"{defect_label} — {size_str} {shape_str} defect, {edge_str} "
            f"(~{px_size}px, aspect ratio: {ar:.1f}, solidity: {sol:.2f}){conf_str}")
...
def run_inference(image_path):
    ...
    Returns:
        dict with keys:
          ...
          reasoning   : str — human-readable explanation (None if NORMAL)
          patch_b64   : base64 PNG of the defect patch (None if NORMAL or unlocalized)
    """
    if _unet is None:
    ...
    if prediction == "ANOMALY":
        ...
        if patch_emb is not None:
            ...
            reasoning = _build_explanation(defect_type, geo_dict, confidence)

            # patch_viz is BGR (from img_norm via cv2.imread) — convert for encoding
            patch_b64 = _img_to_b64(cv2.cvtColor(patch_viz, cv2.COLOR_BGR2RGB))
        else:
            reasoning = "Anomaly detected but defect region too small or could not be localized."

    # ── Print summary to console ──
    print(f"\n{'='*50}")
    print(f"Image     : {image_path}")
    print(f"Score     : {score:.6e}  (threshold: {THRESHOLD:.6e})")
...
if __name__ == "__main__":
    ...
    if result["prediction"] == "ANOMALY":
        ...
        if result["patch_b64"]:
            print("Defect patch returned as base64 PNG (patch_b64).")